...
if local == True:
	...
	socketdb3 = pymysql.connect(host='localhost', port=3306, user='root', passwd='root', db='bull_local')
	...
	socketdb3  = pymysql.connect(host='aggdirect.cflbgllnrj45.us-east-1.rds.amazonaws.com', port=3306, user='bulldog', passwd='AThousandRoads2357', db='aggdirect')
...
def feed(json): 
	...
	for row in results:
		...
		if  t3 <= last_known:
			...
			socketio.emit('ticket', payload)
			socketio.emit('time_now', t3)
	 		print "-----------------TICKET------------------"
			print payload
